...
class MenuService {
  ...
  async removeMenu(menuId: number, email: string) {
    ...
    const menu = await this.menuModel.findMenuByMenuId(menuId);
    let restaurantREGNumber: string | undefined = restaurant?.REGNumber;
    let menuREGNuber: string | undefined = menu?.REGNumber;
    if (!menuREGNuber) throw new Error('해당 메뉴가 없습니다');
    else if (!restaurantREGNumber) throw new Error('Restaurant not found');
    else if (restaurantREGNumber !== menuREGNuber)
      throw new Error('메뉴를 삭제할 권한이 없습니다.');
    else {
